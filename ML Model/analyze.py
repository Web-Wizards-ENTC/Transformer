...
# Attempt to import minimal segmentation API
try:
    from segment_transformer import segment_and_reconstruct
except Exception:
    segment_and_reconstruct = None
...
def draw_bounding_boxes(image, box_info):
    ...
    # Define colors for different fault types
    colors = {
        'loose joint': '#FF0000',      # Red
        'wire overload': '#00FF00',    # Green
        'point overload': '#0000FF',   # Blue
...
def main():
    ...
    try:
        ...
        if segment_and_reconstruct is not None:
        ...
        # Bounding boxes annotation on candidate segmented image
        if res['boxInfo']:
